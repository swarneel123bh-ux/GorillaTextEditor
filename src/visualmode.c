...
// Do all the stuff, including highlighting, copy/cut to clipboard (NEED TO
// DEFINE CLIPBOARD)
int RunVisualMode(void) {
  ...
  while (currentMode == VISUALMODE) {
    ...
    Refresh();
    if (keyhitresult == SIGNAL_SWITCH_TO_NORMALMODE)
      break;
    else if (keyhitresult ==
             SIGNAL_COPY_SELECTED_TEXT_AND_SWITCH_TO_NORMALMODE) {
      Copy(starty, startx, endy, endx);
      break;
    } else if (keyhitresult ==
               SIGNAL_CUT_SELECTED_TEXT_AND_SWITCH_TO_NORMALMODE) {
...
void Copy(int starty, int startx, int endy, int endx) {
  ...
  for (int i = 0; i <= cb.lastIndex; i++) {
    if (i == 0) {
      if (y1 == y2) {
        memcpy(cb.arr[i]->buf, lines.arr[y1 + i]->buf + x1, x2 - x1 + 1);
      } else {
        memcpy(cb.arr[i]->buf, lines.arr[y1 + i]->buf + x1,
               lines.arr[y1 + i]->len - x1);
      }
      ...
      memcpy(cb.arr[i]->buf, lines.arr[y1 + i]->buf,
             lines.arr[y1 + i]->len - 1);
      cb.arr[i]->len += strlen(cb.arr[i]->buf);
    }
  }
...
void Paste(void) {
  ...
  int i;
  for (i = 0; i <= cb.lastIndex - 1;
       i++) { // We are going to print the last line separately to make sure
