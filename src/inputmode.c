...
void cursor_up(void) {
  if (my == 0) {
    ...
    if (my > 0) {
      ...
      my--;
      sx = min((visualmodeactve) ? lines.arr[my]->len - 1 : lines.arr[my]->len,
               sx);
      mx = sx;
    } else {
      mx = 0;
...
void cursor_down(void) {
  if (my == lines.lastIndex) {
    ...
    mvwprintw(imscr, sy, 0, "%s", lines.arr[my]->buf);
    sx = min(sx,
             (visualmodeactve) ? lines.arr[my]->len - 1 : lines.arr[my]->len);
    mx = sx;
  } else {
    my++;
    sy++;
...
void crlf(void) {
  InsertNewLine();
  char copyBuf[1000];
  memset(copyBuf, 0, sizeof(char) * 1000);
  memcpy(copyBuf, (lines.arr[my]->buf + mx),
         sizeof(char) * strlen(lines.arr[my]->buf + mx));
  memset((lines.arr[my]->buf + mx), 0,
         sizeof(char) * strlen(lines.arr[my]->buf + mx));
  wclrtoeol(imscr);
...
void InsertNewLine(void) {
  ...
  sy++;
  wmove(imscr, sy, sx);
  winsertln(imscr); // Inserts new line above current line and pushes everything
                    // else down
  ...
  my--;
  ; // Line we are currently on should not change
  Refresh();
  return;
}
