...
char filename[FILENAME_MAX]; // is what its named, idk where FILENAME_MAX const
                             // comes from stdio.h file
int stdscrstarty, stdscrstartx, stdscrendy, stdscrendx, stdscrnlines,
    stdscrncols;
...
int imscrbgstarty, imscrbgstartx, imscrbgendy, imscrbgendx, imscrbgnlines,
    imscrbgncols;
int cmscrbgstarty, cmscrbgstartx, cmscrbgendy, cmscrbgendx, cmscrbgnlines,
    cmscrbgncols;
...
void Sleep(double seconds) {
  ...
  req.tv_nsec =
      (long)((seconds - req.tv_sec) * 1e9); // Fractional seconds in nanoseconds
...
void WriteToFile(void) {
  ...
  if (!f) {
    ExitProgram(ERR_WRITE_FAIL_FILE_OPEN_FAILURE);
  }
...
void LoadFromFile(void) {
  if (!strcmp(filename, "Untitled")) {
    Message("No file opened as of now");
    return;
  }
...
void PrintError(int errcode) {
  ...
  case ERR_MEM_LINE_ADD_FAIL: {
    printf("Failed to add a line to memory\n");
    break;
  }
  case ERR_UNDEFINED_MODE: {
    printf("Undefined mode variable %d \n", currentMode);
    break;
  }
...
int ProcessKeyhit(void) {
  ...
  case c: {
    ...
    if (currentMode == VISUALMODE) {
      return SIGNAL_COPY_SELECTED_TEXT_AND_SWITCH_TO_NORMALMODE;
    } else {
      InsertInLine('c');
      dirty = true;
      return SIGNAL_SWITCH_TO_INPUTMODE;
  ...
  case x: {
    ...
    if (currentMode == VISUALMODE) {
      return SIGNAL_CUT_SELECTED_TEXT_AND_SWITCH_TO_NORMALMODE;
    } else {
      InsertInLine('c');
      dirty = true;
      return SIGNAL_SWITCH_TO_INPUTMODE;
  ...
  case KEY_UP: {
    cursor_up();
    return (currentMode == NORMALMODE)  ? SIGNAL_SWITCH_TO_NORMALMODE
           : (currentMode == INPUTMODE) ? SIGNAL_SWITCH_TO_INPUTMODE
                                        : SIGNAL_SWITCH_TO_VISUALMODE;
    break;
  }
  case j: { // DOWN
